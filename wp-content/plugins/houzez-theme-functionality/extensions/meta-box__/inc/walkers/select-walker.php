<?php
/**
 * Select Walker
 * For generating Select fields
 */
class RWMB_Select_Walker extends RWMB_Walker
{
	/**
	 * @see Walker::start_el()
	 *
	 * @param string $output            Passed by reference. Used to append additional content.
	 * @param object $object            Item
	 * @param int    $depth             Depth of Item.
	 * @param int    $current_object_id Item id.
	 * @param array  $args
	 */
	public function start_el( &$output, $object, $depth = 0, $args = array(), $current_object_id = 0 )
	{
		$label  = $this->db_fields['label'];
		$id     = $this->db_fields['id'];
		$meta   = $this->meta;
		$indent = str_repeat( "&nbsp;", $depth * 4 );

		$selected = $this->field['std'];

		if ( !empty( $meta[0] ) ) {
			$selected = $meta[0];
		}

		$output .= sprintf(
			'<option value="%s" %s data-value="' . $this->field['std'] . '">%s%s</option>',
			$object->$id,
			/*selected( in_array( $object->$id, $meta ), 1, false )*/( $object->$id == $selected ? 'selected="selected"' : '' ),
			$indent,
			$object->$label
		);
	}
}