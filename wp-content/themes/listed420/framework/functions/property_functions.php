    ...
    function houzez_half_map_listings() {
        ...
        while( $query_args->have_posts() ): $query_args->the_post();
            ...
            if($flag == 0){
                ...
                $property_img = get_post_meta($post_id, 'satellitePropertyImage', true);
                $prop->thumbnail = '<img width="385" height="258" src="'.$property_img.'" class="attachment-houzez-property-thumb-image size-houzez-property-thumb-image wp-post-image" alt="">';
            }
       ...
       $user_role = implode(', ', $user_info->roles);
        ...
        if( $type == 'for_grid_list' ) {
            ...
            if ( !empty( $args[ 'agent_phone' ] ) ) :
                ...
                $output .= '<span><i class="fa fa-phone"></i><span class="clickToShowPhone">' . $args[ 'agent_phone' ] . '</span></span>';

            endif;

            if ( !empty( $args[ 'agent_mobile' ] ) ) :

                $output .= '<span><i class="fa fa-mobile"></i><span class="clickToShow">' . $args[ 'agent_mobile' ] . '</span></span>';
