...
if(isset($_REQUEST['submit_property'])){
	...
	if( (!empty($assessed)) && (!empty($persf)) ) {
		...
		echo "Assessed value and price updated successfully.";
...
if ($_REQUEST['action'] == 'update_property') {
	...
	echo "<br>Property Price Updated Successfully.";
...
echo $assessed = get_option( 'propety_assessed' );
echo " -- ";
echo $persf = get_option( 'propety_persf' );

echo "<br>";

foreach ($metas as $key => $value) {
	# code...
	echo $post_id = $value->post_id;
	echo " -- ";
	...
	// update_post_meta( $value->post_id, 'fave_property_price', 'Steve' );
}
...
<a href="?page=update_price&action=update_property">
<b>Click Here</b></a> to update the property price by Assessed Multiplier(<?php echo get_option( 'propety_assessed' );?>) and Price per SF($<?php echo get_option( 'propety_persf' );?>).
</div>
